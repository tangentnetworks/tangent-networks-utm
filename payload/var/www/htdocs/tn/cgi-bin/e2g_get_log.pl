...
# ---
# Description: e2guardian Log Fetcher - OpenBSD chroot compatible
# Path: ./cgi-bin/e2g_get_log.pl
# ---
use strict;
use warnings;
...
TNSecurity::log_security_event( 'info', 'E2G_E2G_GET_LOG_ACCESS',
    'User ' . ( $session->{username} || 'unknown' ) . ' accessed e2g_get_log' );
...
my $DEBUG_LOG = "/tmp/e2g_get_log-${log_date}.log";
...
debug_log("=== NEW LOG REQUEST ===");

unless ($postdata) {
    debug_log("Empty request");
...
unless ( $action eq 'get_latest_log' ) {
...
{
    debug_log("Pattern validation failed: $pattern");
...
if ( $log_dir =~ m{^([-/\w.]+)$} ) { $log_dir = $1 }
else { send_error( 500, "Invalid log directory path" ) }

debug_log("Log directory: $log_dir");

unless ( -d $log_dir ) {
    debug_log("Directory does not exist: $log_dir");
    send_error( 404, "Log directory not found" );
}

my $latest_log = find_latest_log( $log_dir, $pattern );

unless ($latest_log) {
    debug_log("No matching log files found");
    send_json(
        {
            success     => 0,
...
debug_log("Found log: $latest_log");

my $log_content = read_log_file($latest_log);

if ( defined $log_content && length($log_content) > 0 ) {
    debug_log( "Log loaded: "
          . length($log_content)
          . " bytes  file: "
          . basename($latest_log) );
    send_json(
    ...
    debug_log("Failed to read log or empty file");
    send_json(
        {
            success     => 0,
            message     => "Failed to read log file or log file is empty",
            log_content => '',
            log_file    => ''
        }
    );
...
sub find_latest_log {
    ...
    opendir( my $dh, $dir ) or do {
        debug_log("Failed to open directory: $!");
        return undef;
    };
    ...
    debug_log("Failed to untaint path: $latest");
    return undef;
}
...
sub read_log_file {
    ...
    unless ( $file_path =~ m{^([-/\w.]+)$} ) {
        debug_log("Failed to untaint file path");
        return undef;
    }
    $file_path = $1;
    ...
    debug_log(
        "Read " . ( defined $content ? length($content) : 0 ) . " bytes" );
    return $content;
}
