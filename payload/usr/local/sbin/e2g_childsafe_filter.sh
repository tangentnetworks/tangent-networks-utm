...
# Save as /usr/local/sbin/e2g_childsafe_filter.sh
...
FILTER_TYPE="childsafe"
FILTER_TYPE_UPPER="CHILDSAFE"
LOGFILE="/var/www/htdocs/tn/data/logs/cron/e2guardian-${FILTER_TYPE}-$(date +%Y-%m-%d_%H-%M-%S).log"
FEEDS_CONFIG="/etc/e2guardian/feeds/childsafe.txt"
...
E2GLISTSDIR="/etc/e2guardian/lists/blacklists/${FILTER_TYPE}"
E2GURLS="$E2GLISTSDIR/urls"
E2GDOMAINS="$E2GLISTSDIR/domains"

# Config files
FILTERFILE="/etc/e2guardian/e2guardianf1.conf"
FILTERFILETEMPLATE="/etc/e2guardian/e2guardianf1.conf.c"
...
mkdir -p "$HOSTFILESDIR" "$DOMAINSDIR" "$ADVDIR" "$MIXDIR" "$NEW_FEEDS_DIR" "$E2GLISTSDIR"
...
# Function to parse feeds configuration file
parse_feeds_config() {
  local category=$1
  grep "^${category}:" "$FEEDS_CONFIG" | sed "s/^${category}: *//" | grep -v "^#" | grep -v "^$"
}
...
# Parse feeds from configuration file
echo "$(date '+%Y-%m-%d %H:%M:%S') [INFO] Reading feeds from $FEEDS_CONFIG..." | tee -a "$LOGFILE"

HOSTFILES=$(parse_feeds_config "HOSTFILE")
PORNDOMAINS=$(parse_feeds_config "PORN")
DOMAINSLIST=$(parse_feeds_config "DOMAIN")
MIXLIST=$(parse_feeds_config "MIXED")
ADVERTISERS=$(parse_feeds_config "ADVERTISER")
CAPITOLEARCHIVE=$(parse_feeds_config "CAPITOLE" | head -1)
...
# Download Capitole archive to work directory
if [ -n "$CAPITOLEARCHIVE" ]; then
  ...
  # Merge Capitole URLs and domains
  ...
  if [ -d "${WORKDIR}/blacklists" ]; then
    ...
    echo "$(date '+%Y-%m-%d %H:%M:%S') [WARN] Capitole blacklists directory not found, skipping category processing." | tee -a "$LOGFILE"
  fi
  ...
  echo "$cap_blackdirs" | while read -r cap_dir; do
    ...
    if [ -f "${cap_dir}/domains" ]; then
      if cat "${cap_dir}/domains" >> "$E2GDOMAINS" 2> /dev/null; then
        echo "$(date '+%Y-%m-%d %H:%M:%S') [OK] Capitole blacklist domains: $cap_cat" | tee -a "$LOGFILE"
      else
        echo "$(date '+%Y-%m-%d %H:%M:%S') [ERROR] Failed to read ${cap_dir}/domains" | tee -a "$LOGFILE"
      fi
    fi
    if [ -f "${cap_dir}/urls" ]; then
      if cat "${cap_dir}/urls" >> "$E2GURLS" 2> /dev/null; then
        echo "$(date '+%Y-%m-%d %H:%M:%S') [OK] Capitole blacklist urls: $cap_cat" | tee -a "$LOGFILE"
      else
        echo "$(date '+%Y-%m-%d %H:%M:%S') [ERROR] Failed to read ${cap_dir}/urls" | tee -a "$LOGFILE"
      fi
    fi
  done

  sort -u "$E2GDOMAINS" -o "$E2GDOMAINS"
  sort -u "$E2GURLS" -o "$E2GURLS"
  ...
  echo "$cap_whitedirs" | while read -r cap_dir; do
    ...
    cap_cat=$(basename "$cap_dir")
    if [ -f "${cap_dir}/domains" ]; then
      if cat "${cap_dir}/domains" >> "$CAP_WL_TMP" 2> /dev/null; then
    ...
    if [ -f "${cap_dir}/urls" ]; then
      if cat "${cap_dir}/urls" >> "$CAP_WL_TMP" 2> /dev/null; then
  ...
  if [ -s "$CAP_WL_TMP" ]; then
    ...
    if sort -u "$CAP_WL_TMP" -o "$CAP_WL_TMP" \
      && cat "$CAP_WL_TMP" >> "$E2G_EXCEPTION" \
      && sort -u "$E2G_EXCEPTION" -o "$E2G_EXCEPTION"; then
      echo "$(date '+%Y-%m-%d %H:%M:%S') [OK] Capitole whitelist merged into exception list ($(wc -l < "$CAP_WL_TMP") entries)." | tee -a "$LOGFILE"
    else
      echo "$(date '+%Y-%m-%d %H:%M:%S') [ERROR] Failed to merge Capitole whitelist into exception list." | tee -a "$LOGFILE"
    fi
    rm -f "$CAP_WL_TMP"
  else
...
extract_domains_from_hostfile() {
  ...
  grep -v "^#" "$file" 2> /dev/null | grep . | awk '{print $NF}' | sort -u >> "$RAWDOMAINSLIST"
}
...
extract_domains_from_urls() {
  ...
  sed -E 's|^.*://([^/]+).*|\1|' "$file" | sort -u >> "$RAWDOMAINSLIST"
}
...
extract_domains_from_mixed() {
  ...
  grep -v -E '((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)' "$file" | sort -u >> "$RAWDOMAINSLIST"
}
...
echo "$(date '+%Y-%m-%d %H:%M:%S') [INFO] Domains extracted: $(wc -l < "$RAWDOMAINSLIST")" | tee -a "$LOGFILE"
echo "$(date '+%Y-%m-%d %H:%M:%S') [INFO] Total URLs: $(wc -l < "$E2GURLS")" | tee -a "$LOGFILE"
echo "$(date '+%Y-%m-%d %H:%M:%S') [INFO] Total Domains: $(wc -l < "$E2GDOMAINS")" | tee -a "$LOGFILE"
echo "$(date '+%Y-%m-%d %H:%M:%S') [INFO] ========================================" | tee -a "$LOGFILE"

# Copy to final blocklist location
cp "$E2GDOMAINS" "$DOMAINLIST"

# Setup filter file for childsafe clients
cp "$FILTERFILETEMPLATE" "$FILTERFILE"
...
setup_e2guardian_cron() {
  ...
  if ! cp "$CRONTAB_FILE" "$CRONTAB_FILE.bak"; then
  ...
  if grep -qF "$NEW_JOB" "$CRONTAB_FILE"; then
...
if pgrep e2guardian > /dev/null; then
  ...
  if /usr/local/sbin/e2guardian -c /etc/e2guardian/e2guardian.conf -r; then
  ...
  if /usr/local/sbin/e2guardian -c /etc/e2guardian/e2guardian.conf; then
    sleep 2
    if pgrep e2guardian > /dev/null; then
...
echo "  - Domains: $E2GDOMAINS ($(wc -l < "$E2GDOMAINS") entries)" | tee -a "$LOGFILE"
echo "  - URLs: $E2GURLS ($(wc -l < "$E2GURLS") entries)" | tee -a "$LOGFILE"
